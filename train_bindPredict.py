...
def main():
    ...
    usage_string = 'python train_bindPredict.py evcouplings_res/ snap_res solv_acc/ result.out -m model -r cv_run'
    ...
    parser.add_argument('snap_folder', help='Folder with SNAP2 predictions. '
					    'SNAP-Files need to have the same name in front of the suffix as the EVcouplings folders '
					    '(e.g. Q9XLZ3/ <-> Q9XLZ3.snap2)')
    parser.add_argument('solv_acc_folder', help='Folder with PROFacc predictions. '
						'PROFacc-Files need to have the same name in front of the suffix as the EVcouplings folders '
						'(e.g. Q9XLZ3/ <-> Q9XLZ3.reprof)')
    parser.add_argument('output', help='File to write binding site predictions to')
    parser.add_argument('-m', '--model', help='Input model to use for predictor training', default='mm3_cons_solv_more')
    parser.add_argument('-r', '--run', help='Number of cross-validation run to run')
    parser.add_argument('--snap_suffix', help='Suffix of files in given SNAP-folder (default: "*.snap2")', default='*.snap2')
    ...
    if helper.folder_existence_check(args.evc_folder):	# check if evc folder exists and is reachable
        if helper.folder_existence_check(args.snap_folder):	# check if snap folder exists and is reachable
            if helper.folder_existence_check(args.solv_acc_folder):	# check if solvent accessibility folder exists and is reachable
                predictor = Predictor()
                predictor.train_predictor(args)
