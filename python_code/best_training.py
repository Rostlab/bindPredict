...
train_ids = split1_ids + split2_ids + split3_ids + split4_ids + ids_more_data

cols_to_remove = []
ranges = []
x_train, y_train, train_length, x_cross, y_cross, cross_length = tm.import_model(split1_ids, split2_ids, split3_ids,
                                                                                 split4_ids, split5_ids,
                                                                                 ids_more_data, cols_to_remove,
                                                                                 ranges, keywords=["no", "yes"])
...
train_ids = [i for j, i in enumerate(train_ids) if j not in indices]
...
ranges = []
tm.run_model_best(model, train_ids, split5_ids, cols_to_remove,
                  ranges,
                  words=["no", "no", "yes"])
...
cols_to_remove = [1, 2, 3, 4, 6, 7, 8, 9, 11, 12, 13, 14,
                  16, 17, 18, 19, 21, 22, 23, 24, 26, 27, 28, 29, 31, 32, 33, 34, 36, 37, 38, 39, 40, 41]
ranges = []
tm.run_model_best(model, train_ids, split5_ids, cols_to_remove, ranges,
                  words=["no", "yes", "yes"])
...
ranges = [[0, 0], [1, 2], [3, 4], [5, 5], [6, 7], [8, 9], [10, 10], [11, 12], [13, 14], [15, 15], [16, 17], [18, 19],
          [20, 20], [21, 22], [23, 24], [25, 25], [26, 27], [28, 29], [30, 30], [31, 32], [33, 34], [35, 35], [36, 37],
          [38, 39]]
